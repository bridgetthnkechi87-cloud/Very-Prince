...
export class EmailService {
  ...
  static async sendPayoutNotification(
    ...
    return resend.emails.send({
      from: 'Very-prince <notifications@very-prince.io>',
      to,
      subject: `✨ New Payout: ${amountXlm} XLM allocated from ${orgName}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
          <h1 style="color: #7c3aed;">Hello Maintainer!</h1>
          <p>Great news! <strong>${orgName}</strong> has just allocated a new payout to your wallet on the Very-prince registry.</p>
